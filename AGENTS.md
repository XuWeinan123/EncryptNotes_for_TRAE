...
SealNote（“Seal Note”）是一款基于 SwiftUI 的、采用端到端加密的快速记录便签应用。它通过单一代码库构建并分发为两个 Target：**SealNote** (iOS 17+) 和 **SealNoteMac** (macOS 26+)。

## 构建 / 运行 / 测试

本项目没有 SwiftPM 或 CocoaPods 配置文件 —— 它是一个 Xcode 项目 (`SealNote.xcodeproj`)，使用 Swift 5。
...
- **手动构建 Target：** `xcodebuild -project SealNote.xcodeproj -scheme SealNoteMac -destination 'platform=macOS' build`（或者构建带有 iOS 模拟器目标的 `SealNote` Scheme）。
- **测试：** `SealNoteTests` Target 仅与 **`SealNote`** (iOS) Scheme 绑定。运行命令：`xcodebuild test -project SealNote.xcodeproj -scheme SealNote -destination 'platform=iOS Simulator,name=iPhone 17'`。运行单个测试：在命令后追加 `-only-testing:SealNoteTests/CryptoServiceTests/<method>`。

## 平台隔离

共享代码位于 `SealNote/` 的根目录下。平台特定的代码通过 `#if os(iOS)` / `#if os(macOS)` 以及 `Mac/` 子文件夹进行隔离，子文件夹包括：`App/Mac`、`Views/Mac`、`Stores/Mac`。macOS 版本是一个菜单栏应用（使用 `MacMenuBarController`、`NSStatusItem`），带有悬浮的便签窗口 (`StickyNoteWindow`)；iOS 版本则使用单一的 `WindowGroup` → `ContentView`。在添加新功能时，请先确定它是共享功能还是平台特定功能，然后再放置文件。
...
- 规划文档（`开发计划.md`、`macOS开发计划.md`、`Seal Note_PRD_*.md`）使用中文编写，描述了产品意图和路线图。
- `script/` 是唯一的自动化脚本目录；CI 仅包含 `.github/workflows/auto-merge-owner-prs.yml`。

## 分平台规则

除非用户明确说明要处理 iOS 版本、共享跨平台逻辑，或点名 `SealNote` iOS Scheme，否则默认所有新需求、缺陷修复、构建验证和 UI 调整都针对 macOS 版本（`SealNoteMac`）进行。不要因为项目同时包含 iOS target 就主动扩展到 iOS 侧。
