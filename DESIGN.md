...
产品哲学是「记录想法，而非管理任务」。界面应显得谦逊、密集、几乎没有装饰镀层：清爽的白色索引卡片、一个明亮的叶绿色强调色，以及温暖的近中性灰阶。
...
- **索引卡片，而不是胶囊。** 小巧 3px 圆角、细线边框、几乎不可见的阴影，形成清爽纸感。
- **默认安静。** 低对比度中性色、紧凑排版、无渐变、无发光效果，唯一例外是绿色浮动按钮。
...
| Token                        | Value     | Role                                                  |
| ---------------------------- | --------- | ----------------------------------------------------- |
| `{colors.primary}`           | `#30CF79` | 品牌叶绿色；主按钮、发送、`#tags`、浮动按钮、热力图。 |
| `{colors.primary-container}` | `#E6F9EF` | 带强调色的浅色表面，用于选中状态。                    |
| `{colors.primary-deep}`      | `#397354` | 浅色背景上的强调文字。                                |
| `{colors.link}`              | `#6890F8` | 超链接。                                              |
| `{colors.destructive}`       | `#E47571` | 删除与危险操作。                                      |
| `{colors.ai}`                | `#A94AD9` | AI 洞察强调色。                                       |
| `{colors.pro}`               | `#F07200` | PRO / 会员琥珀色。                                    |
...
| Token                     | Value             | Role                   |
| ------------------------- | ----------------- | ---------------------- |
| `{colors.bg}`             | `#F9F9F9`         | 应用画布。             |
| `{colors.surface-card}`   | `#FFFFFF`         | memo 卡片。            |
| `{colors.surface-raised}` | `#FFFFFF`         | 浮层、底部弹层、菜单。 |
| `{colors.surface-sunken}` | `#F5F5F5`         | 内嵌区域与代码。       |
| `{colors.text-emphasize}` | `#121212`         | 标题与强调内容。       |
| `{colors.text-strong}`    | `#262626`         | 标题文字。             |
| `{colors.text-body}`      | `#2E2E2E`         | 默认正文。             |
| `{colors.text-secondary}` | `#787078`         | 元信息与说明文字。     |
| `{colors.text-subtle}`    | `#949494`         | 时间戳与提示。         |
| `{colors.line}`           | `rgba(0,0,0,.08)` | 极细边框。             |

深色模式使用近黑色表面：`{colors.dark-bg}` 作为画布，`{colors.dark-surface-card}` 作为卡片，保持同一个叶绿色。实现时始终使用语义 token，让主题自动切换。
...
| ---------------------- | ------------------ | ------ | ------------------------------ |
| `{typography.caption}` | 13px / 16px        | 400    | 时间戳、元信息、列标题。       |
| `{typography.body}`    | 14px / 20px        | 400    | 默认 memo 内容。               |
| `{typography.body-lg}` | 15px / 22px        | 400    | 舒适阅读与设置行。             |
| `{typography.title}`   | 16px / 24px        | 600    | 分区 / 分组标题。              |
| `{typography.page}`    | 20px / 28px        | 600    | 屏幕 / 导航栏标题。            |
...
| L2 Card     | `0 1px 2px rgba(51,44,96,.08)` + L1。                      | 默认 memo 卡片。               |
| L3 Popover  | `0 6px 24px rgba(0,0,0,.12), 0 0 0 .5px rgba(0,0,0,.06)`。 | 浮层、菜单、底部弹层、对话框。 |
| L4 Float    | `0 4px 14px rgba(48,207,121,.36)`。                        | 唯一允许绿色光晕的浮动按钮。   |
...
形状语言是小圆角、纸感、克制。卡片、按钮和输入框默认使用 `{rounded.sm}` 3px；界面表面应像索引卡片，而不是柔软胶囊。

| Token            | Value  | Use                            |
| ---------------- | ------ | ------------------------------ |
| `{rounded.sm}`   | 3px    | 卡片、按钮、输入框。           |
| `{rounded.md}`   | 6px    | 浮层、菜单、底部弹层、对话框。 |
| `{rounded.lg}`   | 12px   | 大型底部弹层与图片。           |
| `{rounded.full}` | 9999px | 仅限浮动按钮、圆形头像、chip。 |
...
`{components.button-primary}` 是唯一主动作样式：叶绿色背景、白色文字、3px 圆角。用于发送、主 CTA 和关键确认动作。

`{components.button-tonal}` 用于选中或弱强调状态：浅绿色背景、深绿色文字。普通工具按钮应保持中性色。

### Memo & Tags

`{components.memo-card}` 是默认内容容器：白色表面、柔和黑正文、16px 内边距、3px 圆角、0.5px 边框和低调阴影。卡片操作（分享、更多）应默认隐藏，悬停时渐进披露。

`{components.tag-inline}` 用于 memo 正文中的 `#tags`，继承正文节奏并使用叶绿色文字。侧栏 chip 可以使用 `{rounded.full}`，但正文标签不应显得像大型胶囊按钮。

### Navigation & Feedback

`FloatButton` 是系统中唯一允许绿色光晕的元素。`NavBar` 高度为 52px。`Snackbar`、`Dialog`、菜单和底部弹层使用 `{rounded.md}` 与 L3 浮层阴影。
